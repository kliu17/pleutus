...
const int BUFF_SIZE = 10240;   // optimized for laptop, but can be reset to other buffer size
const int MIN_MESSAGE_SIZE = 11;
const int MAX_MESSAGE_SIZE = 88;
...
struct cnt_r{
    ...
    struct buffer*  incomplete_buff = buffer_new(MAX_MESSAGE_SIZE); // hold imcomplete message.
};
...
int main() {
    ...
    int overhead_pages = ((file_size / BUFF_SIZE) * MAX_MESSAGE_SIZE) / BUFF_SIZE;
    int pages = file_size / BUFF_SIZE + overhead_pages ;

    int count_pages = 0;
    struct buffer* buff =  buffer_new(BUFF_SIZE);

    buffer_read(buff, fd);
    cout << "buffer_size " << buff->capacity << endl << endl;
    while (true) {
	...
	if (res.find(stream_id) != res.end()){ // stream not first time appear
		...
		msg_len = ntohs(buffer_get_le16(res[stream_id].incomplete_buff));
		...
		if (package_size != msg_len +2) { //first incomplete package
		    package_append(res[stream_id].incomplete_buff, buff->data+buff->start-2, package_size);
		    buffer_advance(buff, package_size-2); //advance buffer
		    //update flag
		    res[stream_id].is_complete_package = false;
		} else { //complete message
               	    ouch_out_message_decode(buff, message,msg_type,  shares);
                    if(msg_type == OUCH_MSG_EXECUTED){
                        cout << shares << " executed shares" << endl;
                        res = update_cnt_executed(res, (int)stream_id, int(shares));
                    } else
	    ...
	    if(package_size != msg_len +2){ //incomplete package
                package_append(res[stream_id].incomplete_buff, buff->data+buff->start, package_size-2);
                buffer_advance(buff, package_size-2); //advance buffer
                res[stream_id].is_complete_package = false;
            } else { //complete message
